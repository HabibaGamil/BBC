...
import com.example.post_mongo.DataClasses.ViewRequest;
import com.example.post_mongo.commands.getArticleCommand;
...
Producer producer;
    ...
    public Response consumeMQServerMessage(Request request){
        try {
            ...
            if(obj instanceof getArticleCommand) {
                Method execute = c.getMethod("execute", String.class);
                Map<String, Object> reqBody = request.getBody();
                Article article = (Article) execute.invoke(obj, reqBody.get("id"));

                ViewRequest viewRequest = ViewRequest.builder()
                        .categoryId(article.getCategory().get(0))
                        .build();
               // producer.sendMessage(viewRequest);
            }
        } catch (Exception e) {
...
import com.example.post_mongo.DataClasses.Response;
...
public class Consumer {
    ...
    void setProperties () throws ClassNotFoundException {
        if(this.properties==null){
            properties=new Properties(config.getMsg(),config.getCmdMap());
        }
    }
    @RabbitListener(queues = "${rabbitmq.queue.mqServer.name}")
    public Response consumeMQServerMessage(Request request){
